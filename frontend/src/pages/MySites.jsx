                        ...
                        <div className="relative aspect-video bg-slate-100 dark:bg-slate-800 overflow-hidden shrink-0">
                            ...
                            <div className="w-full h-full overflow-hidden bg-white relative group-hover:scale-105 transition-transform duration-500">
                                {project.status === 'failed' ? (
                                    <div className="w-full h-full flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-800/50 p-6 text-center gap-3">
                                        <p className="text-sm text-slate-500 dark:text-slate-400">
                                            {project.error || 'Build failed. Please try again.'}
                                        </p>
                                        <button
                                            onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleRetryBuild(project.projectId); }}
                                    ...
                                    <img
                                        src={`${project.previewUrl}?t=${project.updatedAt?.seconds || Date.now()}`}
